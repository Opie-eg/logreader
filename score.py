...
def updateUserScore(userscore,computer,notitype):
    ...
    if (logValues == ""):
        ...
        f = open("statistics.txt", "w+", encoding= "utf-8")  # open file in write mode
        f.write(str(newTypeLogValues))
        f.close()
    else:
        #obj = { key: undefined };
        #print("key" in obj); 
        if(CURRENTYEAR in logValues):
            if(logValues[CURRENTYEAR][computer]) :
                ...
                newLogValues[CURRENTYEAR][computer][ALLMONTHS[CURRENTMONTH]][notitype]+= score
